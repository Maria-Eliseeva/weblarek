...
- `card:select` - выбор карточки товара в каталоге. Открывает её в модальном окне
- `card:buy` - добавление в корзину в превью товара
- `card:delete` - удаление товара из корзины

События корзины:
- `basket:select` - открытие корзины в модальном окне 
- `basket:makeOrder` - клик по кнопке Оформить. Переход к формам 

События модального окна:
- `modal:close` - закрытие модального окна

События формы заказа:
- `formOrder:change` - изменение данных в форме заказа
- `cardPay:select` - выбор оплаты картой
- `cashPay:select` - выбор оплаты наличными
- `address:input` - ввод адреса доставки
- `nextform:open` - переход ко 2 части формы заполнения контактов

События формы контактов:
- `email:input` - ввод email
- `phone:input` - ввод телефона 
- `formContacts:change` - изменение данных в форме контактов
- `nextform:submit` - отправка формы контактов и открытие окна завершения заказа

### Презентер

Презентер реализован в основном файле приложения `main.ts` без вынесения в отдельный класс. Выбран подход без выделения отдельного класса презентера, так как приложение имеет одну страницу. 

Основные компоненты:
- `catalogModel` - модель каталога товаров
- `basketModel` - модель корзины
- `buyerModel` - модель данных покупателя
- `productService` - сервис для работы с API товаров
- `events` -  брокер событий


Используются мапы для связывания DOM-элементов с данными (`elementCatalogToProduct`, `productToPreview`, `basketToProduct`)
Эти дополнительные функции помогают реализовать быструю связь между слоями Model и View.
